...
@section('content')
  ...
  <div class="content-header">
      <div class="header-section">
        <h4>
            <i class="gi gi-sort"> </i> Progress Report <br>
        </h4>
      </div>
  </div>
  <ol class="breadcrumb breadcrumb-top">
      <li><a href="{{ route('pm.home') }}"><i class="fa fa-home"></i></a></li>
      <li><a>Progress Report</a></li>
  </ol>
   <div class="block">
    {{ Form::open(['target' => '_blank','url'=>'pm/progressreport/printProgress']) }}
               
                <div class="row">
                 <div class="col-md-4 col-md-offset-1">
                    <label for="quotation">Choose Contract </label> 
                        <select name="contract" id="contract" class='form-control select-chosen ' required="required" data-placeholder='Choose'>
                        <option value=""></option>
                        @foreach($contract as $contract)
                          <option value="{{$contract->conid}}">{{$contract->name}}</option>
                        @endforeach
                      </select> 
                 </div>
                  <div class="col-md-6">
